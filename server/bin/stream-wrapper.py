...
# Simply overrides the defaults when necessary, and the ingestionAddress is the
  ...
  insert_stream_response = youtube.liveStreams().insert(
    ...
    body=dict(
      ...
      cdn=dict(
        format="720p",
        ingestionType="rtmp",
        ingestionInfo=dict(ingestionAddress="https://1drv.ms/v/s!AnLNeqMkjLFdgVYR6uP8Np6xOGVG")
      )
    )
  ).execute()
